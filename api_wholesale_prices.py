"""
Run-only. Queries half-hourly GB wholesale electricity prices from Elexon Insights Market Index Data (MID).
Run: python api_wholesale_prices.py → data/api_wholesale_prices.xlsx.

Pulls GB wholesale electricity prices from the Elexon Insights MID dataset via the public REST API, 
and reduces a full calendar year to an average half-hourly day per season × day-type.
...
Usage:
    python api_wholesale_prices.py                
...
from seasons import MONTH_SEASON_BY_NUM as MONTH_SEASON, SEASONS, DAY_TYPES, HH_PER_DAY
...
# MONTH_SEASON / SEASONS / DAY_TYPES / HH_PER_DAY come from seasons.py, shared with the demand model.
...
def write_workbook(profiles, annual_mean_mwh, n_obs, year, provider, path=WHOLESALE_PRICES_XLSX):
    ...
    ws.cell(row=2, column=1, value=(f"Pulled for calendar {year}. Each cell = mean MID price over all days of that "
            ...
            "Generated by api_wholesale_prices.py; re-run to refresh."))
    hdr = 4
...
def _load(path=WHOLESALE_PRICES_XLSX):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Wholesale price data not found at {path}. Run 'python api_wholesale_prices.py' first to generate it."
        )

...
Pulls ERA5 hourly 2 m air temperature from the Open-Meteo Historical Weather API for each district's 
representative station, and reduces a multi-year window to an average 24-hour day per season
(the climatological mean diurnal cycle).The model adds an hourly anomaly onto the HDD-derived 
...
Notes / caveats:
  - GB timezone.
  - Window defaults to the last 10 complete calendar years (→ 2016-2025 today), matching the sunshine
    baseline (datasets.SUNSHINE_BASELINE_YEARS), so the climatology is not driven by one odd year.
  - ERA5 has ~5-day delay; the latest complete year is always fully available.
  - Pulled year-by-year per district (resilient, modest response sizes); ~9 districts × N years calls.

Usage:
    python api_temperature_profiles.py                 # last 10 complete years → data/api_temperature_profiles.xlsx
...
def build_profiles(times, temps):
    """Mean ERA5 temperature [°C] per (season, local clock hour), dropping nulls.
...
def write_workbook(district_profiles, start_year, end_year, path=TEMPERATURE_PROFILES_XLSX):
    ...
    ws = wb.active
    ws.title = "HourlyTemp"
    ws.cell(row=1, column=1, value=(f"ERA5 mean 2 m air temperature [°C] by district × season × local clock hour, "
            f"{start_year}-{end_year}. Generated by temperature_profiles.py — do not hand-edit; re-run to refresh."))
    hdr = 3
